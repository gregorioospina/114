...
import { EXTERNAL_ARTICLES } from "@/constants/articles";
...
export const getStaticProps: GetStaticProps<ArticleProps> = async ({ params }) => {
	...
	const articleMeta = EXTERNAL_ARTICLES.find((article) => article.id === id);
...
const Article = ({ content, title, description, author }: ArticleProps) => {
	return (
		<>
			<Head>
				...
				<div className="container flex flex-col mx-auto px-4 max-w-3xl">
					...
					<div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-12 pb-8 border-b-2 border-foreground">
						<div className="flex flex-col text-center md:text-left">
							...
							<p className="text-xl uppercase tracking-wider font-light opacity-90">{author}</p>
						</div>
					</div>
